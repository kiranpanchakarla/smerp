															...
															<c:forEach items="${list}" var="list">
																<tr>
                                                                    ...
                                                                    <c:when test="${fn:containsIgnoreCase(permissions,'Multilevel Approval')}">  <a class="btn btn-delete" href="#"
                                                                        onclick="deleteById('<c:out value="${list.id}"/>','/gr/delete')" data-toggle="tooltip" data-placement="top" title="Delete"><i
																			...
																			<c:when test="${list.status != 'Approved' && purchaseRequestsList.status != 'Partially_Approved' && list.status != 'Cancelled'  && list.status != 'Rejected'  && list.status != 'Partially_Returned'  && list.status != 'Goods_Return' && list.status != 'Invoiced'}">
																				<c:forEach items="${sessionScope.umpmap}" var="ump">
																					<c:if test="${ump.key eq 'Goods Receipt'}">
																						<c:set var="permissions" scope="session"
																							value="${ump.value}" />
