												...
												<div class="card-block card-dashboard">
													...
													<div class="table-responsive">
														<table id="example" class="display nowrap table table_padding_custom table-hover table-striped table-bordered"
															...
															<tbody>
															<c:forEach items="${purchaseRequestsList}" var="purchaseRequestsList">
																<tr>
																<td><c:set var="count" value="${count + 1}" scope="page" />
                                                                     <c:out value="${count}" />  </td>
																	<td>${purchaseRequestsList.user.username}</td>
																	<td>${purchaseRequestsList.user.userEmail}</td>
																	<td>${purchaseRequestsList.docNumber}</td>
																	<td>${purchaseRequestsList.postingDate}</td>
																	<%-- <th><c:if test="${productList.isActive eq true}">Active</c:if>
																	<c:if test="${productList.isActive eq false}">InActive</c:if></th> --%>
																	<td >
																	 <a class ="btn btn-edit" href="<c:url value="/purchaseReq/getInfo?purchaseReqId=${purchaseRequestsList.id}"/>"><i class="icon-edit left"></i></a>
																	<a  class ="btn btn-delete" href="#"  onclick="deleteById('<c:out value="${purchaseRequestsList.id}"/>','/purchaseReq/delete')"><i class="icon-bin left"></i></a>
				                                                    <a  class ="btn btn-view" href="<c:url value="/purchaseReq/view?purchaseReqId=${purchaseRequestsList.id}"/>"><i class="icon-eye3 left"></i></a>
																	</td>
																</tr>
																</c:forEach>
															</tbody>
