										...
										<div class="card-header" style="height: 60px;">
											<div class="row">
												...
												<c:if test="${ump.key eq 'Admin Master'}">
														...
														href="<c:url value="/currency/create"/>">Create</a>
													    </c:if>
												</c:if>
												</c:forEach>
												</div>
											...
											<div class="card-block card-dashboard">
												...
												<c:set var="permissions" scope="session" value="${ump.value}" />
														<c:if test="${fn:containsIgnoreCase(permissions,'update')}">
																	<a
																		class="btn btn-edit"
																		href="<c:url value="/currency/getInfo?currencyId=${currency.id}"/>" data-toggle="tooltip" data-placement="right" title="Edit">
														...
														<c:if test="${fn:containsIgnoreCase(permissions,'delete')}">
																	  <a class="btn btn-delete mr-1" href="#"
																		onclick="deleteById('<c:out value="${currency.id}"/>','/currency/delete')" data-toggle="tooltip" data-placement="right" title="Delete"><i
														...
														<c:if test="${fn:containsIgnoreCase(permissions,'view')}">				
																			 <a
																		class="btn btn-view"
																		href="<c:url value="/currency/view?currencyId=${currency.id}"/>" data-toggle="tooltip" data-placement="right" title="View"><i
