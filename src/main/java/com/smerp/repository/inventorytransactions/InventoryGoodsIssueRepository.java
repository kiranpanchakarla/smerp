...
public interface InventoryGoodsIssueRepository extends JpaRepository<InventoryGoodsIssue,Integer> {
	...
	@Query("SELECT r FROM InventoryGoodsIssue r WHERE isActive=:isActive and plant.id in (:plantIds) order by createdAt Desc")
	List<InventoryGoodsIssue> findByIsActive(Boolean isActive, int[] plantIds);
