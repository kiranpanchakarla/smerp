...
<body data-open="click" data-menu="vertical-menu" data-col="2-columns"
								...
								<div class="content-body">
									...
									<div class="card">
										<div class="card-header" style="height: 60px;">
											<div class="row">
												<div class="col-md-4">
													<h2 class="content-header-title">Request for Quotation</h2>
												</div>
												<div class="col-md-4">
														...
														<tbody>
															<c:forEach items="${list}" var="list">
																<tr>
																	...
																	<td><c:choose>
																			<c:when test="${list.status != 'Approved' && list.status != 'ConvertedToPO' && list.status != 'Rejected' && list.status != 'Cancelled'}">
																				<c:forEach items="${sessionScope.umpmap}" var="ump">
																					<c:if test="${ump.key eq 'RFQ'}">
																						<c:set var="permissions" scope="session"
																							value="${ump.value}" />
																						<c:choose>
																							<c:when
																								...
																								<a class="btn btn-edit"
																									href="<c:url value="/rfq/edit?id=${list.id}"/>" data-toggle="tooltip" data-placement="top" title="Edit"><i
	...
	function getSearchFilterList() {
 	...
 	function downloadExcelFile(){		
		...
		a.href = "<c:url value='/rfq/exportRFQExcel?'/>"+stringQuery;
	} 
